 ...
 * @retval -1 Error
 */
int mutt_do_pager(struct PagerView *view)
{
  assert(view);
  assert(view->data);
  assert(view->data->fname);
  assert(view->mode == PAGER_MODE_ATTACH || view->mode == PAGER_MODE_OTHER);
  ...
  else
  {
    ...
    mutt_endwin();
    mutt_buffer_file_expand_fmt_quote(cmd, c_pager, view->data->fname);
    if (mutt_system(mutt_buffer_string(cmd)) == -1)
    {
      ...
      rc = 0;
    mutt_file_unlink(view->data->fname);
    mutt_buffer_pool_release(&cmd);
  }
