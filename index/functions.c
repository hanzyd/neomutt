 ...
 */
static enum IndexRetval op_check_stats(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_check_traditional(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_delete(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
 ...
 */
static enum IndexRetval op_delete_thread(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
 ...
 */
static enum IndexRetval op_edit_raw_message(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_end_cond(struct Menu *menu, int op, struct IndexData *idata)
{
  return IR_SUCCESS;
}
 ...
 */
static enum IndexRetval op_exit(struct Menu *menu, int op, struct IndexData *idata)
{
  idata->close = op;
 ...
 */
static enum IndexRetval op_extract_keys(struct Menu *menu, int op, struct IndexData *idata)
{
  if (!WithCrypto)
    return IR_NOT_IMPL;
  struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
 ...
 */
static enum IndexRetval op_forget_passphrase(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_jump(struct Menu *menu, int op, struct IndexData *idata)
{
  char buf[PATH_MAX] = { 0 };
  int msg_num = 0;
  if (isdigit(LastKey))
 ...
 */
static enum IndexRetval op_mailbox_list(struct Menu *menu, int op, struct IndexData *idata)
{
  mutt_mailbox_list();
  return IR_VOID;
}
 ...
 */
static enum IndexRetval op_mail_key(struct Menu *menu, int op, struct IndexData *idata)
{
  if (!(WithCrypto & APPLICATION_PGP))
    return IR_NOT_IMPL;
 ...
 */
static enum IndexRetval op_main_break_thread(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
 ...
 */
static enum IndexRetval op_main_change_folder(struct Menu *menu, int op, struct IndexData *idata)
{
  ...
  if (c_change_folder_next && idata->mailbox &&
 ...
 */
static enum IndexRetval op_main_collapse_all(struct Menu *menu, int op, struct IndexData *idata)
{
  const short c_sort = cs_subset_sort(idata->sub, "sort");
  if ((c_sort & SORT_MASK) != SORT_THREADS)
  {
 ...
 */
static enum IndexRetval op_main_collapse_thread(struct Menu *menu, int op,
 ...
 */
static enum IndexRetval op_main_delete_pattern(struct Menu *menu, int op,
                                               struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
  /* L10N: Due to the implementation details we do not know whether we
 ...
 */
static enum IndexRetval op_main_limit(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_main_link_threads(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
 ...
 */
static enum IndexRetval op_main_next_new(struct Menu *menu, int op, struct IndexData *idata)
{
  int first_unread = -1;
  int first_new = -1;
 ...
 */
static enum IndexRetval op_main_next_undeleted(struct Menu *menu, int op,
                                               struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_main_next_unread_mailbox(struct Menu *menu, int op,
                                                    struct IndexData *idata)
{
  struct Mailbox *m = idata->mailbox;
 ...
 */
static enum IndexRetval op_main_prev_undeleted(struct Menu *menu, int op,
                                               struct IndexData *idata)
{
  if (menu->current < 1)
  {
 ...
 */
static enum IndexRetval op_main_read_thread(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
  /* L10N: Due to the implementation details we do not know whether we
 ...
 */
static enum IndexRetval op_main_set_flag(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_main_show_limit(struct Menu *menu, int op, struct IndexData *idata)
{
  if (!ctx_has_limit(idata->ctx))
 ...
 */
static enum IndexRetval op_main_sync_folder(struct Menu *menu, int op, struct IndexData *idata)
{
  ...
  if ((menu->current < 0) || (idata->mailbox && (menu->current >= idata->mailbox->vcount)))
  {
    menu->current = ci_first_message(idata->sub, idata->mailbox);
  }

  /* check for a fatal error, or all messages deleted */
  if (idata->mailbox && mutt_buffer_is_empty(&idata->mailbox->pathbuf))
  {
    ctx_free(&Contex2);
    idata->ctx = NULL;
  }
 ...
 */
static enum IndexRetval op_main_undelete_pattern(struct Menu *menu, int op,
                                                 struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
  /* L10N: Due to the implementation details we do not know whether we
 ...
 */
static enum IndexRetval op_menu_move(struct Menu *menu, int op, struct IndexData *idata)
{
  switch (op)
  {
    case OP_BOTTOM_PAGE:
 ...
 */
static enum IndexRetval op_next_entry(struct Menu *menu, int op, struct IndexData *idata)
{
  if (menu->current >= (idata->mailbox->vcount - 1))
  {
    if (!idata->in_pager)
 ...
 */
static enum IndexRetval op_prev_entry(struct Menu *menu, int op, struct IndexData *idata)
{
  if (menu->current < 1)
  {
    if (!idata->in_pager)
 ...
 */
static enum IndexRetval op_query(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_quit(struct Menu *menu, int op, struct IndexData *idata)
{
  idata->close = op;
  if (idata->attach_msg)
  {
 ...
 */
static enum IndexRetval op_redraw(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_save(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_search(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_shell_escape(struct Menu *menu, int op, struct IndexData *idata)
{
  if (mutt_shell_escape())
  {
    mutt_mailbox_check(idata->mailbox, MUTT_MAILBOX_CHECK_FORCE);
  }
 ...
 */
static enum IndexRetval op_show_log_messages(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_sort(struct Menu *menu, int op, struct IndexData *idata)
{
  if (mutt_select_sort((op == OP_SORT_REVERSE)) != 0)
    return IR_ERROR;

  if (idata->mailbox && (idata->mailbox->msg_count != 0))
  {
 ...
 */
static enum IndexRetval op_toggle_write(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_undelete_thread(struct Menu *menu, int op, struct IndexData *idata)
{
  /* L10N: CHECK_ACL */
  /* L10N: Due to the implementation details we do not know whether we
     undelete zero, 1, 12, ... messages. So in English we use
 ...
 */
static enum IndexRetval op_version(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_what_key(struct Menu *menu, int op, struct IndexData *idata)
{
  mutt_what_key();
  return IR_VOID;
}
 ...
 */
static enum IndexRetval op_autocrypt_acct_menu(struct Menu *menu, int op,
                                               struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_main_imap_fetch(struct Menu *menu, int op, struct IndexData *idata)
{
  if (idata->mailbox && (idata->mailbox->type == MUTT_IMAP))
 ...
 */
static enum IndexRetval op_main_imap_logout_all(struct Menu *menu, int op,
                                                struct IndexData *idata)
{
  if (idata->mailbox && (idata->mailbox->type == MUTT_IMAP))
  {
 ...
 */
static enum IndexRetval op_catchup(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_get_children(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_get_message(struct Menu *menu, int op, struct IndexData *idata)
{
  char buf[PATH_MAX] = { 0 };
  if (idata->mailbox->type == MUTT_NNTP)
  {
 ...
 */
static enum IndexRetval op_main_change_group(struct Menu *menu, int op, struct IndexData *idata)
{
  bool pager_return = true; /* return to display message in pager */
  struct Buffer *folderbuf = mutt_buffer_pool_get();
  mutt_buffer_alloc(folderbuf, PATH_MAX);
  ...
  OptNews = true;
  const char *const c_news_server = cs_subset_string(idata->sub, "news_server");
  CurrentNewsSrv =
 ...
 */
static enum IndexRetval op_main_entire_thread(struct Menu *menu, int op, struct IndexData *idata)
{
  char buf[PATH_MAX] = { 0 };
 ...
 */
static enum IndexRetval op_main_vfolder_from_query(struct Menu *menu, int op,
                                                   struct IndexData *idata)
{
  char buf[PATH_MAX] = { 0 };
 ...
 */
static enum IndexRetval op_main_windowed_vfolder_backward(struct Menu *menu, int op,
                                                          struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_main_windowed_vfolder_forward(struct Menu *menu, int op,
                                                         struct IndexData *idata)
{
  const short c_nm_query_window_duration =
      cs_subset_number(idata->sub, "nm_query_window_duration");
 ...
 */
static enum IndexRetval op_sidebar_next(struct Menu *menu, int op, struct IndexData *idata)
{
 ...
 */
static enum IndexRetval op_sidebar_open(struct Menu *menu, int op, struct IndexData *idata)
{
  struct MuttWindow *win_sidebar = mutt_window_find(dlg, WT_SIDEBAR);
 ...
 */
static enum IndexRetval op_sidebar_toggle_visible(struct Menu *menu, int op,
                                                  struct IndexData *idata)
{
