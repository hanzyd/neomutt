...
static int ibar_repaint(struct MuttWindow *win)
{
  ...
  menu_status_line(buf + debug_len, sizeof(buf) - debug_len, idata->ctx->menu,
                   idata->mailbox, NONULL(c_status_format));
  mutt_window_move(win, 0, 0);
  ...
  if (c_ts_enabled && TsSupported)
  {
    ...
    menu_status_line(buf, sizeof(buf), idata->ctx->menu, idata->mailbox,
