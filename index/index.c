 ...
 * unread message.
 */
static int ci_first_message(struct ConfigSubset *sub, struct Mailbox *m)
{
  if (!m || (m->msg_count == 0))
    return 0;
 ...
 * @param menu Current Menu
 */
static void resort_index(struct ConfigSubset *sub, struct Context *ctx, struct Menu *menu)
{
  ...
  if (menu->current < 0)
    menu->current = ci_first_message(sub, ctx->mailbox);
 ...
 */
static void update_index_threaded(struct ConfigSubset *sub, struct Context *ctx,
                                  enum MxStatus check, int oldcount)
{
  ...
  const bool c_uncollapse_new = cs_subset_bool(sub, "uncollapse_new");
 ...
 */
static void update_index(struct ConfigSubset *sub, struct Menu *menu,
                         struct Context *ctx, enum MxStatus check, int oldcount,
                         const struct CurrentEmail *cur)
{
  ...
  if ((c_sort & SORT_MASK) == SORT_THREADS)
    update_index_threaded(sub, ctx, check, oldcount);
  else
  ...
  if (menu->current < 0)
  {
    menu->current = (old_current < ctx->mailbox->vcount) ?
                        old_current :
                        ci_first_message(sub, ctx->mailbox);
  }
}
 ...
 * @param menu      Current Menu
 * @param ctx       Mailbox
 * @param check     Flags, e.g. #NeoMutt->MX_STATUS_REOPENED
 * @param oldcount  How many items are currently in the index
 * @param cur_email Currently selected email
 *
 * @note cur_email cannot be NULL
 */
void mutt_update_index(struct ConfigSubset *sub, struct Menu *menu, struct Context *ctx,
                       enum MxStatus check, int oldcount, const struct Email *cur_email)
{
  struct CurrentEmail se = { .e = NULL, .sequence = cur_email->sequence };
  update_index(sub, menu, ctx, check, oldcount, &se);
}
                                  ...
                                  const struct CurrentEmail *cur, bool read_only)
{
  ...
  struct IndexData *idata = menu->mdata;

  struct Mailbox *m_ctx = ctx_mailbox(idata->ctx);
    ...
    *oldcount = m_ctx->msg_count;
    ...
    if (check != MX_STATUS_OK)
    {
      ...
      if ((check == MX_STATUS_NEW_MAIL) || (check == MX_STATUS_REOPENED))
        update_index(idata->sub, menu, idata->ctx, check, *oldcount, cur);
  ...
  idata->ctx = mx_mbox_open(m, flags);
  Contex2 = idata->ctx;
  if (idata->ctx)
  {
    menu->current = ci_first_message(idata->sub, idata->ctx->mailbox);
#ifdef USE_INOTIFY
    mutt_monitor_add(NULL);
#endif
  }
  ...
  const bool c_collapse_all = cs_subset_bool(idata->sub, "collapse_all");
  ...
  struct EventMailbox em = { ctx_mailbox(idata->ctx) };
 ...
 * @param read_only    Open Mailbox in read-only mode
 */
static void change_folder_string(struct ConfigSubset *sub, struct Menu *menu,
...
#endif
  {
    const char *const c_folder = cs_subset_string(sub, "folder");
...
void index_make_entry(struct Menu *menu, char *buf, size_t buflen, int line)
{
  ...
  const char *const c_index_format =
      cs_subset_string(idata->sub, "index_format");
  mutt_make_string(buf, buflen, menu->win_index->state.cols, NONULL(c_index_format),
                   m, idata->ctx->msg_in_pager, e, flags, NULL);
}
...
static void index_custom_redraw(struct Menu *menu)
{
  ...
  if (menu->redraw & REDRAW_STATUS)
  {
    ...
    const char *const c_status_format =
        cs_subset_string(idata->sub, "status_format");
...
int mutt_index_menu(struct MuttWindow *dlg)
{
  ...
  menu->current = idata->ctx ? ci_first_message(idata->sub, idata->ctx->mailbox) : 0;
  ...
  while (true)
  {
    ...
    {
      resort_index(idata->sub, idata->ctx, menu);
    }
    ...
    if (ctx_mailbox(idata->ctx))
    {
               ...
               (check == MX_STATUS_FLAGS))
      {
        ...
        else if (check == MX_STATUS_NEW_MAIL)
        {
          for (size_t i = 0; i < idata->ctx->mailbox->msg_count; i++)
          {
            ...
            if (e && !e->read && !e->old)
            {
              ...
              const bool c_beep_new = cs_subset_bool(idata->sub, "beep_new");
              if (c_beep_new)
                mutt_beep(true);
              const char *const c_new_mail_command =
                  cs_subset_string(idata->sub, "new_mail_command");
        ...
        update_index(idata->sub, menu, idata->ctx, check, idata->oldcount,
                     &idata->cur);
    ...
    else
    {
      ...
      const bool c_arrow_cursor = cs_subset_bool(idata->sub, "arrow_cursor");
      const bool c_braille_friendly =
          cs_subset_bool(idata->sub, "braille_friendly");
      if (c_arrow_cursor)
      ...
      const bool c_auto_tag = cs_subset_bool(idata->sub, "auto_tag");
...
#ifdef USE_NOTMUCH
    if (idata->ctx)
      nm_db_debug_check(idata->ctx->mailbox);
#endif
    ...
    switch (op)
    {
...
#ifdef USE_NNTP
      case OP_GET_PARENT:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE))
          break;
        /* fallthrough */

      case OP_GET_MESSAGE:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_READONLY | CHECK_ATTACH))
          break;
        char buf[PATH_MAX] = { 0 };
        if (idata->ctx->mailbox->type == MUTT_NNTP)
        {
          ...
          if (!idata->ctx->mailbox->id_hash)
            idata->ctx->mailbox->id_hash = mutt_make_id_hash(idata->ctx->mailbox);
          struct Email *e = mutt_hash_find(idata->ctx->mailbox->id_hash, buf);
          if (e)
          {
            if (e->vnum != -1)
            {
            ...
            else if (e->collapsed)
            {
              ...
              mutt_set_vnum(idata->ctx->mailbox);
              menu->current = e->vnum;
              menu->redraw = REDRAW_MOTION_RESYNC;
            }
            else
          ...
          else
          {
            ...
            int rc = nntp_check_msgid(idata->ctx->mailbox, buf);
            if (rc == 0)
            {
              e = idata->ctx->mailbox->emails[idata->ctx->mailbox->msg_count - 1];
              mutt_sort_headers(idata->ctx->mailbox, idata->ctx->threads, false,
                                &idata->ctx->vsize);
              menu->current = e->vnum;
              menu->redraw = REDRAW_FULL;
            }
      ...
      case OP_RECONSTRUCT_THREAD:
      {
        ...
        char buf[PATH_MAX] = { 0 };
        int oldmsgcount = idata->ctx->mailbox->msg_count;
        ...
        if (op == OP_RECONSTRUCT_THREAD)
        {
          ...
          {
            if (!mutt_hash_find(idata->ctx->mailbox->id_hash, ref->data))
            {
              rc = nntp_check_msgid(idata->ctx->mailbox, ref->data);
              if (rc < 0)
                break;
            }
        ...
        if (rc >= 0)
          rc = nntp_check_children(idata->ctx->mailbox, buf);

        /* at least one message has been loaded */
        if (idata->ctx->mailbox->msg_count > oldmsgcount)
        {
          struct Email *e_oldcur = mutt_get_virt_email(idata->ctx->mailbox, menu->current);
          bool verbose = idata->ctx->mailbox->verbose;

          if (rc < 0)
            idata->ctx->mailbox->verbose = false;
          mutt_sort_headers(idata->ctx->mailbox, idata->ctx->threads,
      ...
      case OP_JUMP:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX))
          break;
        char buf[PATH_MAX] = { 0 };
        int msg_num = 0;
        if (isdigit(LastKey))
        ...
        else if ((msg_num < 1) || (msg_num > idata->ctx->mailbox->msg_count))
          mutt_error(_("Invalid message number"));
        else if (!idata->ctx->mailbox->emails[msg_num - 1]->visible)
          mutt_error(_("That message is not visible"));
        else
        {
          struct Email *e = idata->ctx->mailbox->emails[msg_num - 1];

          if (mutt_messages_in_thread(idata->ctx->mailbox, e, MIT_POSITION) > 1)
          {
            mutt_uncollapse_thread(e);
            mutt_set_vnum(idata->ctx->mailbox);
          }
      ...
      case OP_MAIN_DELETE_PATTERN:
        ...
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_DELETE, _("Can't delete messages")))
          break;
      ...
      case OP_MAIN_FETCH_MAIL:
        if (!prereq(idata->ctx, menu, CHECK_ATTACH))
          break;
        pop_fetch_mail();
        menu->redraw = REDRAW_FULL;
        break;
      ...
      case OP_TOGGLE_READ:
      {
        ...
        if (op == OP_TOGGLE_READ)
        {
          ...
          mutt_pattern_func(idata->ctx, MUTT_LIMIT, NULL);
        }

        if (((op == OP_LIMIT_CURRENT_THREAD) &&
             mutt_limit_current_thread(idata->ctx, idata->cur.e)) ||
            (op == OP_TOGGLE_READ) ||
            ((op == OP_MAIN_LIMIT) &&
             (mutt_pattern_func(idata->ctx, MUTT_LIMIT, _("Limit to messages matching: ")) == 0)))
        {
          if (menu->oldcurrent >= 0)
          {
            /* try to find what used to be the current message */
            menu->current = -1;
            for (size_t i = 0; i < idata->ctx->mailbox->vcount; i++)
            {
              struct Email *e = mutt_get_virt_email(idata->ctx->mailbox, i);
              if (!e)
                continue;
            ...
            menu->current = 0;
          const short c_sort = cs_subset_sort(idata->sub, "sort");
          if ((idata->ctx->mailbox->msg_count != 0) && ((c_sort & SORT_MASK) == SORT_THREADS))
          {
            const bool c_collapse_all =
                cs_subset_bool(idata->sub, "collapse_all");
            if (c_collapse_all)
              collapse_all(idata->ctx, menu, 0);
            mutt_draw_tree(idata->ctx->threads);
          }
      ...
      case OP_QUIT:
      {
        ...
        const enum QuadOption c_quit = cs_subset_quad(idata->sub, "quit");
        if (query_quadoption(c_quit, _("Quit NeoMutt?")) == MUTT_YES)
        {
          idata->oldcount =
              (idata->ctx && idata->ctx->mailbox) ? idata->ctx->mailbox->msg_count : 0;
          ...
          if (!idata->ctx || ((check = mx_mbox_close(&Contex2)) == MX_STATUS_OK))
          {
            idata->ctx = NULL;
            idata->done = true;
          }
      ...
      case OP_SEARCH:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX))
          break;
        menu->current =
            mutt_search_command(idata->ctx, idata->ctx->mailbox, menu->current, op);
        if (menu->current == -1)
      ...
      case OP_SORT_REVERSE:
        ...
        if (ctx_mailbox(idata->ctx) && (idata->ctx->mailbox->msg_count != 0))
        {
          resort_index(idata->sub, idata->ctx, menu);
          OptSearchInvalid = true;
        }
      ...
      case OP_TAG:
      {
        ...
        {
          struct Mailbox *m = idata->ctx->mailbox;
        ...
        else
        {
          ...
          if (c_resolve && (menu->current < idata->ctx->mailbox->vcount - 1))
          {
            menu->current++;
            menu->redraw |= REDRAW_MOTION_RESYNC;
          }
      ...
      case OP_MAIN_UNDELETE_PATTERN:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        /* L10N: Due to the implementation details we do not know whether we
            undelete zero, 1, 12, ... messages. So in English we use
            "messages". Your language might have other means to express this. */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_DELETE, _("Can't undelete messages")))
          break;

        if (mutt_pattern_func(idata->ctx, MUTT_UNDELETE,
      ...
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_ATTACH))
          break;
      ...
      case OP_MAIN_IMAP_FETCH:
        if (ctx_mailbox(idata->ctx) && (idata->ctx->mailbox->type == MUTT_IMAP))
          imap_check_mailbox(idata->ctx->mailbox, true);
        break;
      ...
      case OP_MAIN_SYNC_FOLDER:
        ...
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_READONLY))
          break;
        {
          int ovc = idata->ctx->mailbox->vcount;
          int oc = idata->ctx->mailbox->msg_count;
          struct Email *e = NULL;

          /* don't attempt to move the cursor if there are no visible messages in the current limit */
          if (menu->current < idata->ctx->mailbox->vcount)
          {
            ...
            if (idata->cur.e->deleted)
              newidx = ci_next_undeleted(idata->ctx->mailbox, menu->current);
            if (newidx < 0)
              newidx = ci_previous_undeleted(idata->ctx->mailbox, menu->current);
            if (newidx >= 0)
              e = mutt_get_virt_email(idata->ctx->mailbox, newidx);
          }

          enum MxStatus check = mx_mbox_sync(idata->ctx->mailbox);
          if (check == MX_STATUS_OK)
          {
            if (e && (idata->ctx->mailbox->vcount != ovc))
            {
              for (size_t i = 0; i < idata->ctx->mailbox->vcount; i++)
              {
                struct Email *e2 = mutt_get_virt_email(idata->ctx->mailbox, i);
                if (e2 == e)
                {
                  menu->current = i;
                  break;
          ...
          {
            update_index(idata->sub, menu, idata->ctx, check, oc, &idata->cur);
          }

          /* do a sanity check even if mx_mbox_sync failed.  */

          if ((menu->current < 0) ||
              (ctx_mailbox(idata->ctx) && (menu->current >= idata->ctx->mailbox->vcount)))
          {
            menu->current = ci_first_message(idata->sub, idata->ctx->mailbox);
          }
        }
        ...
        if (ctx_mailbox(idata->ctx) && mutt_buffer_is_empty(&idata->ctx->mailbox->pathbuf))
        {
          ctx_free(&Contex2);
          idata->ctx = NULL;
        }
      ...
      case OP_MAIN_QUASI_DELETE:
        ...
        else
        {
          ...
          idata->ctx->mailbox->changed = true;
        }
        break;
      ...
      case OP_MAIN_ENTIRE_THREAD:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE))
          break;
        char buf[PATH_MAX] = { 0 };
        if (idata->ctx->mailbox->type != MUTT_NOTMUCH)
        {
          if (((idata->ctx->mailbox->type != MUTT_MH) &&
               (idata->ctx->mailbox->type != MUTT_MAILDIR)) ||
          ...
          if (idata->ctx->mailbox->msg_count == 0)
          {
        ...
        idata->oldcount = idata->ctx->mailbox->msg_count;
        struct Email *e_oldcur = mutt_get_virt_email(idata->ctx->mailbox, menu->current);
        if (nm_read_entire_thread(idata->ctx->mailbox, e_oldcur) < 0)
        {
          mutt_message(_("Failed to read thread, aborting"));
          break;
        }
        if (idata->oldcount < idata->ctx->mailbox->msg_count)
        {
      ...
      case OP_MAIN_MODIFY_TAGS_THEN_HIDE:
      {
        ...
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        if (!idata->cur.e)
          break;
        char *tags = NULL;
        ...
        else
        {
          ...
          if (c_resolve)
          {
            menu->current = ci_next_undeleted(idata->ctx->mailbox, menu->current);
            if (menu->current == -1)
            {
              menu->current = menu->oldcurrent;
              menu->redraw = REDRAW_CURRENT;
      ...
      case OP_CHECK_STATS:
        mutt_check_stats(ctx_mailbox(idata->ctx));
        break;
      ...
      case OP_MAIN_WINDOWED_VFOLDER_BACKWARD:
      {
        ...
        const short c_nm_query_window_duration =
            cs_subset_number(idata->sub, "nm_query_window_duration");
        ...
        const char *const c_nm_query_window_current_search =
            cs_subset_string(idata->sub, "nm_query_window_current_search");
...
#endif
      {
        ...
        char *cp = NULL;
        bool read_only;
        const bool c_read_only = cs_subset_bool(idata->sub, "read_only");
        ...
        const bool c_change_folder_next =
            cs_subset_bool(idata->sub, "change_folder_next");
        if (c_change_folder_next && ctx_mailbox(idata->ctx) &&
            !mutt_buffer_is_empty(&idata->ctx->mailbox->pathbuf))
        {
          mutt_buffer_strcpy(folderbuf, mailbox_path(idata->ctx->mailbox));
          mutt_buffer_pretty_mailbox(folderbuf);
        }
        /* By default, fill buf with the next mailbox that contains unread mail */
        mutt_mailbox_next(idata->ctx ? idata->ctx->mailbox : NULL, folderbuf);

        if (mutt_buffer_enter_fname(cp, folderbuf, true, ctx_mailbox(idata->ctx),
        ...
        {
          change_folder_string(idata->sub, menu, folderbuf->data, folderbuf->dsize,
      ...
      case OP_MAIN_CHANGE_GROUP_READONLY:
      {
        ...
        CurrentNewsSrv = nntp_select_server(idata->ctx ? idata->ctx->mailbox : NULL,
                                            c_news_server, false);
        if (!CurrentNewsSrv)
          goto changefoldercleanup2;

        nntp_mailbox(idata->ctx ? idata->ctx->mailbox : NULL, folderbuf->data,
                     folderbuf->dsize);
      ...
      case OP_DISPLAY_HEADERS: /* don't weed the headers */
      {
        ...
        if (op == OP_DISPLAY_HEADERS)
          bool_str_toggle(idata->sub, "weed", NULL);
            ...
            (idata->tag || !(idata->cur.e->security & PGP_TRADITIONAL_CHECKED)))
        {
          struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
          el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
          mutt_check_traditional_pgp(ctx_mailbox(idata->ctx), &el, &menu->redraw);
          emaillist_clear(&el);
        }
        set_current_email(&idata->cur,
                          mutt_get_virt_email(idata->ctx->mailbox, menu->current));
        ...
        menu->oldcurrent = menu->current;
        if (ctx_mailbox(idata->ctx))
          update_index(idata->sub, menu, idata->ctx, MX_STATUS_NEW_MAIL,
                       idata->ctx->mailbox->msg_count, &idata->cur);
        continue;
      }
      ...
      case OP_EXIT:
            ...
            (query_quadoption(c_quit, _("Exit NeoMutt without saving?")) == MUTT_YES))
        {
          if (idata->ctx)
          {
            mx_fastclose_mailbox(idata->ctx->mailbox);
            ctx_free(&Contex2);
            idata->ctx = NULL;
          }
          idata->done = true;
      ...
      case OP_MAIN_BREAK_THREAD:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_WRITE, _("Can't break thread")))
          break;
        if (!idata->cur.e)
          break;
        ...
        {
          {
            ...
            mutt_sort_headers(idata->ctx->mailbox, idata->ctx->threads, true,
                              &idata->ctx->vsize);
      ...
      case OP_MAIN_LINK_THREADS:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_WRITE, _("Can't link threads")))
          break;
        if (!idata->cur.e)
          break;
        ...
        else
        {
          ...
          if (mutt_link_threads(idata->cur.e, &el, idata->ctx->mailbox))
          {
            mutt_sort_headers(idata->ctx->mailbox, idata->ctx->threads, true,
                              &idata->ctx->vsize);
            menu->current = idata->cur.e->vnum;
      ...
      case OP_MAIN_NEXT_UNDELETED:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE))
          break;
        if (menu->current >= (idata->ctx->mailbox->vcount - 1))
        {
          if (!idata->in_pager)
      ...
      case OP_MAIN_PREV_UNDELETED:
        ...
        menu->current = ci_previous_undeleted(idata->ctx->mailbox, menu->current);
        if (menu->current == -1)
        {
          menu->current = menu->oldcurrent;
          if (!idata->in_pager)
      ...
      case OP_DECODE_SAVE:
      {
        ...
        const int rc = mutt_save_message(idata->ctx->mailbox, &el, save_opt, transform_opt);
      ...
      case OP_MAIN_PREV_NEW_THEN_UNREAD:
      {
        ...
        for (size_t i = 0; i != idata->ctx->mailbox->vcount; i++)
        {
          ...
          {
            mcur++;
            if (mcur > (idata->ctx->mailbox->vcount - 1))
            {
              mcur = 0;
            }
          }
          else
          {
            ...
            if (mcur < 0)
            {
              mcur = idata->ctx->mailbox->vcount - 1;
            }
          }

          struct Email *e = mutt_get_virt_email(idata->ctx->mailbox, mcur);
          if (!e)
            break;
          const short c_sort = cs_subset_sort(idata->sub, "sort");
      ...
      case OP_FLAG_MESSAGE:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_WRITE, _("Can't flag message")))
          break;
      ...
      case OP_TOGGLE_NEW:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_SEEN, _("Can't toggle new")))
          break;
      ...
      case OP_TOGGLE_WRITE:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX))
          break;
        if (mx_toggle_write(idata->ctx->mailbox) == 0)
        {
          if (idata->in_pager)
          {
            op = OP_DISPLAY_MESSAGE;
      ...
      case OP_MAIN_CLEAR_FLAG:
      {
        ...
        if (mutt_change_flag(idata->ctx->mailbox, &el, (op == OP_MAIN_SET_FLAG)) == 0)
        {
          menu->redraw |= REDRAW_STATUS;
          const bool c_resolve = cs_subset_bool(idata->sub, "resolve");
          if (idata->tag)
      ...
      case OP_BOUNCE_MESSAGE:
      {
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        ci_bounce_message(idata->ctx->mailbox, &el);
        emaillist_clear(&el);
        break;
      }
      ...
      case OP_DELETE:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_DELETE, _("Can't delete message")))
          break;

        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);

        mutt_emails_set_flag(idata->ctx->mailbox, &el, MUTT_DELETE, true);
        mutt_emails_set_flag(idata->ctx->mailbox, &el, MUTT_PURGE, (op == OP_PURGE_MESSAGE));
        const bool c_delete_untag = cs_subset_bool(idata->sub, "delete_untag");
        if (c_delete_untag)
          mutt_emails_set_flag(Contex2->mailbox, &el, MUTT_TAG, false);
        emaillist_clear(&el);
      ...
      case OP_PURGE_THREAD:
      {
        ...
        int rc = mutt_thread_set_flag(ctx_mailbox(idata->ctx), idata->cur.e,
                                      MUTT_DELETE, true, subthread);
        if (rc == -1)
          break;
        if (op == OP_PURGE_THREAD)
        {
          rc = mutt_thread_set_flag(ctx_mailbox(idata->ctx), idata->cur.e,
        ...
        const bool c_delete_untag = cs_subset_bool(idata->sub, "delete_untag");
        if (c_delete_untag)
          mutt_thread_set_flag(ctx_mailbox(idata->ctx), idata->cur.e, MUTT_TAG,
                               false, subthread);
        const bool c_resolve = cs_subset_bool(idata->sub, "resolve");
        if (c_resolve)
        {
...
#ifdef USE_NNTP
      case OP_CATCHUP:
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_READONLY | CHECK_ATTACH))
          break;
        if (idata->ctx && (idata->ctx->mailbox->type == MUTT_NNTP))
        {
          struct NntpMboxData *mdata = idata->ctx->mailbox->mdata;
          if (mutt_newsgroup_catchup(idata->ctx->mailbox, mdata->adata, mdata->group))
            menu->redraw = REDRAW_INDEX | REDRAW_STATUS;
        }
        break;
#endif
      ...
      case OP_ENTER_COMMAND:
        ...
        window_set_focus(win_index);
        if (idata->ctx)
          mutt_check_rescore(idata->ctx->mailbox);
        menu->redraw = REDRAW_FULL;
        break;
      ...
      case OP_VIEW_RAW_MESSAGE:
      {
        ...
        {
          if (!prereq(idata->ctx, menu, CHECK_READONLY))
            break;
          /* L10N: CHECK_ACL */
          if (!check_acl(idata->ctx->mailbox, MUTT_ACL_INSERT, _("Can't edit message")))
            break;
          edit = true;
        }
        else if (op == OP_EDIT_OR_VIEW_RAW_MESSAGE)
          edit = !idata->ctx->mailbox->readonly &&
                 (idata->ctx->mailbox->rights & MUTT_ACL_INSERT);
        else
          edit = false;
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        mutt_ev_message(idata->ctx->mailbox, &el, edit ? EVM_EDIT : EVM_VIEW);
        emaillist_clear(&el);
        menu->redraw = REDRAW_FULL;
      ...
      case OP_EDIT_LABEL:
      {
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        int num_changed = mutt_label_message(idata->ctx->mailbox, &el);
        emaillist_clear(&el);
      ...
      case OP_LIST_REPLY:
      {
        ...
        mutt_send_message(SEND_REPLY | SEND_LIST_REPLY, NULL, NULL, idata->ctx,
                          &el, idata->sub);
        emaillist_clear(&el);
        menu->redraw = REDRAW_FULL;
        break;
      ...
      case OP_EXTRACT_KEYS:
      {
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        crypt_extract_keys_from_messages(idata->ctx->mailbox, &el);
        emaillist_clear(&el);
        menu->redraw = REDRAW_FULL;
        break;
      }
      ...
      case OP_PIPE:
      {
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        mutt_pipe_message(idata->ctx->mailbox, &el);
        emaillist_clear(&el);
        ...
        if ((idata->ctx->mailbox->type == MUTT_IMAP) && !c_imap_peek)
        {
      ...
      case OP_PRINT:
      {
        ...
        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
        mutt_print_message(idata->ctx->mailbox, &el);
        emaillist_clear(&el);
      ...
      case OP_MAIN_READ_SUBTHREAD:
      {
        ...
        /* L10N: Due to the implementation details we do not know whether we
           ...
           "messages". Your language might have other means to express this. */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_SEEN, _("Can't mark messages as read")))
          break;

        int rc = mutt_thread_set_flag(ctx_mailbox(idata->ctx), idata->cur.e,
                                      MUTT_READ, true, (op != OP_MAIN_READ_THREAD));
        if (rc != -1)
        {
          const bool c_resolve = cs_subset_bool(idata->sub, "resolve");
          if (c_resolve)
      ...
      case OP_POST:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_ATTACH))
          break;
        if (!idata->cur.e)
          break;
                              ...
                              _("Reply by mail as poster prefers?")) != MUTT_YES))
        {
          const enum QuadOption c_post_moderated =
              cs_subset_quad(idata->sub, "post_moderated");
          if (idata->ctx && (idata->ctx->mailbox->type == MUTT_NNTP) &&
              !((struct NntpMboxData *) idata->ctx->mailbox->mdata)->allowed && (query_quadoption(c_post_moderated, _("Posting to this group not allowed, may be moderated. Continue?")) != MUTT_YES))
          {
            break;
          }
          if (op == OP_POST)
            mutt_send_message(SEND_NEWS, NULL, NULL, idata->ctx, NULL, idata->sub);
          else
          {
            if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT))
              break;
            struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
            el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);
      ...
      case OP_UNDELETE:
      {
        if (!prereq(idata->ctx, menu, CHECK_IN_MAILBOX | CHECK_MSGCOUNT | CHECK_VISIBLE | CHECK_READONLY))
          break;
        /* L10N: CHECK_ACL */
        if (!check_acl(idata->ctx->mailbox, MUTT_ACL_DELETE, _("Can't undelete message")))
          break;

        struct EmailList el = STAILQ_HEAD_INITIALIZER(el);
        el_add_tagged(&el, idata->ctx, idata->cur.e, idata->tag);

        mutt_emails_set_flag(idata->ctx->mailbox, &el, MUTT_DELETE, false);
        mutt_emails_set_flag(idata->ctx->mailbox, &el, MUTT_PURGE, false);
        emaillist_clear(&el);
      ...
      case OP_UNDELETE_SUBTHREAD:
      {
        ...
        int rc = mutt_thread_set_flag(ctx_mailbox(idata->ctx), idata->cur.e, MUTT_DELETE,
      ...
      case OP_SIDEBAR_TOGGLE_VISIBLE:
        bool_str_toggle(idata->sub, "sidebar_visible", NULL);
        mutt_window_reflow(NULL);
        break;
#endif
      ...
      case OP_AUTOCRYPT_ACCT_MENU:
        dlg_select_autocrypt_account(ctx_mailbox(idata->ctx));
        break;
#endif
