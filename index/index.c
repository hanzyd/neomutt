 ...
 * unread message.
 */
int ci_first_message(struct ConfigSubset *sub, struct Mailbox *m)
{
  if (!m || (m->msg_count == 0))
    return 0;
 ...
 * @param menu Current Menu
 */
void resort_index(struct ConfigSubset *sub, struct Context *ctx, struct Menu *menu)
{
 ...
 */
void update_index(struct ConfigSubset *sub, struct Menu *menu, struct Context *ctx,
                  enum MxStatus check, int oldcount, const struct CurrentEmail *cur)
{
  if (!menu || !ctx)
    return;
 ...
 * @param read_only    Open Mailbox in read-only mode
 */
void change_folder_string(struct ConfigSubset *sub, struct Menu *menu, char *buf,
                          size_t buflen, int *oldcount, const struct CurrentEmail *cur,
